		      ...
		      <tr>
		        ...
		        <td>
		        	<input type="button" value="문의하기" id="btn"  
		        		 onclick="location.href='/member/myinquiry?m_id=${vo.m_id}&c_no=${vo.c_no }'">
		        </td>
		      </tr>
	      </c:forEach>
	  </tbody>
		      ...
		      <tr>
		        ...
		        <td>
		        	...
		        	<c:if test="${vo2.p_status == 'paid' }">
			        	...
			        	onclick="location.href='/member/myreview?c_no=${vo2.c_no }&p_no=${vo2.p_no }'">
		        	</c:if>
		        </td>
		      </tr>
	      </c:forEach>
	...
	<!-- 페이징처리  -->
		<div class="pagination">
			<ul>
				...
				<li><a id="prev" href="/member/classList?pageNum=${amap.startPage-1}">이전</a></li>
				</c:if>
				<c:forEach var="i" begin="${amap.startPage }" end="${amap.endPage }">
				<li><a class="pageNumbers" href="/member/classList?pageNum=${i}">${i }</a></li>
				</c:forEach>
				<c:if test="${amap.pageCount > amap.endPage }">
				<li><a id="next" href="/member/classList?pageNum=${amap.endPage+1}">다음</a></li>
				</c:if>
			</ul>
	    </div>
	<!-- 페이징처리  -->
